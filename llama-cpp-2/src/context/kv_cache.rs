...
impl LlamaContext {
    ...
    pub fn copy_cache(&mut self, src: i32, dest: i32, size: i32) {
        unsafe { llama_cpp_sys_2::llama_kv_cache_seq_cp(self.context.lock().unwrap().as_ptr(), src, dest, 0, size) }
    }
    ...
    pub fn copy_kv_cache_seq(
        ...
        unsafe {
            llama_cpp_sys_2::llama_kv_cache_seq_cp(self.context.lock().unwrap().as_ptr(), src, dest, p0, p1);
        }
        Ok(())
    }
    ...
    pub fn kv_cache_clear_seq(
        ...
        let p1 = p1
            .map_or(Ok(-1), i32::try_from)
            .map_err(KvCacheConversionError::P1TooLarge)?;
        Ok(unsafe { llama_cpp_sys_2::llama_kv_cache_seq_rm(self.context.lock().unwrap().as_ptr(), src, p0, p1) })
    }
    ...
    pub fn kv_cache_seq_rm(
        ...
        Ok(
            unsafe {
                llama_cpp_sys_2::llama_kv_cache_seq_rm(self.context.lock().unwrap().as_ptr(), seq_id, p0, p1)
            },
        )
    }
    ...
    pub fn get_kv_cache_used_cells(&self) -> i32 {
        unsafe { llama_cpp_sys_2::llama_get_kv_cache_used_cells(self.context.lock().unwrap().as_ptr()) }
    }

     /// Clear the KV cache
     pub fn kv_cache_clear(&mut self) {
        unsafe { llama_cpp_sys_2::llama_kv_cache_clear(self.context.lock().unwrap().as_ptr()) }
    }
    ...
    pub fn llama_kv_cache_seq_keep(&mut self, seq_id: i32) {
        unsafe { llama_cpp_sys_2::llama_kv_cache_seq_keep(self.context.lock().unwrap().as_ptr(), seq_id) }
    }
    ...
    pub fn kv_cache_seq_add(
        ...
        let p1 = p1
            ...
            .map_err(KvCacheConversionError::P1TooLarge)?;
        unsafe {
            llama_cpp_sys_2::llama_kv_cache_seq_add(self.context.lock().unwrap().as_ptr(), seq_id, p0, p1, delta);
        }
        Ok(())
    }
    ...
    pub fn kv_cache_seq_div(
        ...
        unsafe { llama_cpp_sys_2::llama_kv_cache_seq_div(self.context.lock().unwrap().as_ptr(), seq_id, p0, p1, d) }
        Ok(())
    }
    ...
    pub fn kv_cache_seq_pos_max(&self, seq_id: i32) -> i32 {
        unsafe { llama_cpp_sys_2::llama_kv_cache_seq_pos_max(self.context.lock().unwrap().as_ptr(), seq_id) }
    }
    ...
    pub fn kv_cache_defrag(&mut self) {
        unsafe { llama_cpp_sys_2::llama_kv_cache_defrag(self.context.lock().unwrap().as_ptr()) }
    }

    /// Apply the KV cache updates (such as K-shifts, defragmentation, etc.)
    pub fn kv_cache_update(&mut self) {
        unsafe { llama_cpp_sys_2::llama_kv_cache_update(self.context.lock().unwrap().as_ptr()) }
    }
    ...
    pub fn get_kv_cache_token_count(&self) -> i32 {
        unsafe { llama_cpp_sys_2::llama_get_kv_cache_token_count(self.context.lock().unwrap().as_ptr()) }
    }
    ...
    pub fn new_kv_cache_view(&self, n_max_seq: i32) -> KVCacheView {
        let view =
            unsafe { llama_cpp_sys_2::llama_kv_cache_view_init(self.context.lock().unwrap().as_ptr(), n_max_seq) };
...
impl KVCacheView<'_> {
    ...
    pub fn update(&mut self) {
        unsafe {
            llama_cpp_sys_2::llama_kv_cache_view_update(self.ctx.context.lock().unwrap().as_ptr(), &mut self.view);
        }
    }
